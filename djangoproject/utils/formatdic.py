...
print "Before cleaning there are ", len(data), " users"
...
def clean_empty_values(d, n):
	"""
	"""
	return dict([(k, v) for k,v in d.iteritems() if len(v)>0][:n])	
...
write_json("data/tags.json", tags)
print "Writing tags to tags.json"

# --------- End to refactor ------

data = filter_tags(data, tags)
data = clean_empty_values(data, 100)
print "After cleaning there are ", len(data), "users"
